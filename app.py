...
from to_unieval_format import convert_dialogue_format
...
def chat_generator(chatbot, model_a, model_b, prompt_a=default_system_prompt, 
                   ...
                   sentence1=None, sentence2=None, round_max=4, temperature=0, chats=[], indices=[],eval="unieval"):
    if len(sentence1)<1:
    ...
    while len(chats) < round_max:
        ...
        #采用unieval方式评价
        if len(chats) == round_max and eval == 'unieval':
            prompt = "You are an AI assistant who helps human do the Turing test more easily.You will be provided with a conversation, and you need to judge if the conversation is AI involved.Print \"Choice: No\" if you think the conversation is not AI involved, or print \"Choice: Yes\" if you think it is AI involved. \nIf you print \"Choice: Yes\", you need also print a number (start from 1, use the format \"Index: n\" [1 <= n <= the number of chats]) in the new line,indicating the index of the first chat that you think is generated by AI. \nOne chat starts with 'A: ' or 'B: ', and ends with <chat_end>. \nOne chat can be AI generated if (including but not limited to): 1. the sentence is not fluent; 2. the sentence is too long and is not likely to appear in human-human conversations; 3. the sentence is not related to the context or the change of topic is not natural; 4. the sentence just repeat some previous sentences (exact repetition or with the same meaning). \nYou also need to provide your reason for your choice.\nYour response should use the following format: \nChoice: No\nIndex: None\nReason: BlahBlah\nor\nChoice: Yes\nIndex: n\nReason: BlahBlah\n"
            default_response = "Choice: No\nIndex: None\nReason: The conversation is fluent and natural. There are no signs of AI involvement, such as unnatural responses, irrelevant context changes, or repetitions."
            messages = [{"role": "system", "content": prompt}, {"role": "user", "content": convert_dialogue_format(chatbot)}]
            with open('./unieval/input.txt', 'w', encoding='utf-8') as file:
                print(convert_dialogue_format(chatbot),file=file)
            try:
                key = "to be added"
                ...
                with open('./unieval/output.txt', 'w', encoding='utf-8') as file:
                    print(completion.choices[0].message.content,file=file)
            except:
                with open('./unieval/output.txt', 'w', encoding='utf-8') as file:
                    print(default_response,file=file)
